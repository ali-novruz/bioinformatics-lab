...
from biolab.sensitivity import nested_expression_cv,permutation_pvalue,singleton_enrichment
...
def count_inputs(tmp_path, second='g2\tI\t11\t20\t+\t10\t4\ng1\tI\t1\t10\t+\t10\t3\n'):
    ...
    (tmp_path/'a.tsv').write_text(header+'g1\tI\t1\t10\t+\t10\t1\ng2\tI\t11\t20\t+\t10\t2\n')
...
@pytest.mark.parametrize('change', ['coordinate','fraction','missing','duplicate','overflow'])
def test_counts_reject_incompatible_or_invalid_inputs(tmp_path,change):
    source=count_inputs(tmp_path)
    path=tmp_path/'b.tsv';text=path.read_text()
    replacements={'coordinate':('g1\tI\t1','g1\tII\t1'),'fraction':('\t3\n','\t3.5\n'),
                  'missing':('\t3\n','\t\n'),'duplicate':('g2\t','g1\t'),
                  'overflow':('\t3\n','\t9223372036854775808\n')}
    path.write_text(text.replace(*replacements[change]))
    with pytest.raises(ValueError):combine_feature_counts(source)
...
def test_singleton_exact_known_answer_and_resolution():
    ...
    assert np.isclose(result.min_padj.min(),.02)
    assert permutation_pvalue(.9,[.1]*99)==.01
    assert permutation_pvalue(.9,[.9]*99)==1
...
def test_nested_cv_known_signal_and_partition_boundaries(monkeypatch):
    ...
    rng=np.random.default_rng(7);y=np.repeat([0,1],20)
    x=rng.normal(size=(40,8));x[:,0]=y*20+rng.normal(0,.01,40)
    x[:,-1]=np.arange(40)
    folds,pred=nested_expression_cv(x,y,outer_folds=4,inner_folds=3,
                                   grid={'select__k':[1,2],'classifier__C':[.1]})
...
def test_failed_run_is_sealed_and_corruption_detected(tmp_path):
    external=tmp_path/'input.txt';external.write_text('input')
    ...
    with pytest.raises(ValueError,match='checksum'):verify_outputs(out,record)


def test_registry_rejects_duplicate_id(tmp_path):
    (tmp_path/'projects').mkdir();(tmp_path/'scripts').mkdir()
    (tmp_path/'scripts/run.py').write_text('pass')
    (tmp_path/'docs.md').write_text('project')
    p={'id':'valid','title':'Valid','script':'run.py','args':[],'offline':True,'fetch':[],'docs':'docs.md'}
    (tmp_path/'projects/registry.json').write_text(json.dumps({'projects':[p,p]}))
    with pytest.raises(ValueError,match='duplicate'):load_registry(tmp_path)


def test_cli_unknown_project_and_list():
    listed=subprocess.run([sys.executable,str(ROOT/'scripts/lab.py'),'list'],capture_output=True,text=True)
    assert listed.returncode==0 and len(listed.stdout.splitlines())==12
    invalid=subprocess.run([sys.executable,str(ROOT/'scripts/lab.py'),'run','does-not-exist'],capture_output=True,text=True)
...
def test_custom_rna_plot_names_and_design(tmp_path,monkeypatch):
    spec=importlib.util.spec_from_file_location('audit_run_projects',ROOT/'scripts/run_projects.py')
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    from matplotlib.figure import Figure
    titles=[]
    monkeypatch.setattr(Figure,'savefig',lambda self,*a,**kw: titles.extend(ax.get_title() for ax in self.axes))
    norm=pd.DataFrame([[1,3,5],[3,7,2],[7,1,9],[9,2,4]],index=['a','b','c','d'])
    meta=pd.DataFrame({'condition':['treated','treated','untreated','untreated']},index=norm.index)
    res=pd.DataFrame({'padj':[.01,.5,1.],'log2FoldChange':[2.,-.1,0.]})
    module.plot_expression(res,norm,meta,tmp_path,dataset='Yeast chrI',design='~ condition')
...
def test_suite_does_not_mark_a_missing_result_successful(tmp_path,monkeypatch):
    (tmp_path/'scripts').mkdir();(tmp_path/'projects').mkdir()
    (tmp_path/'scripts/no_result.py').write_text('print("no result")')
    (tmp_path/'docs.md').write_text('fixture')
    (tmp_path/'projects/registry.json').write_text(json.dumps({'projects':[
        {'id':'empty','title':'Empty','script':'no_result.py','args':[],'offline':True,'fetch':[],'docs':'docs.md'}]}))
    ...
    report=json.loads(next((tmp_path/'results/runs').rglob('suite.json')).read_text())
...
def test_research_graph_rejects_broken_relation(tmp_path):
    ...
    graph={'nodes':[{'id':'a','status':'protocol','path':'projects/README.md'}],
           'edges':[{'from':'missing','to':'a','relation':'invalid'}]}
    (tmp_path/'research/graph.json').write_text(json.dumps(graph))
    with pytest.raises(ValueError,match='endpoints'):check_health(tmp_path)
