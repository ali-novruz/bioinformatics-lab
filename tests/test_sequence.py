...
from biolab.sequence import clean_dna,reverse_complement,gc_content,motif_positions,align,motif_enrichment,seed_search
...
def test_dp_exhaustive_against_biopython(mode):
    ...
    for a in words:
        for b in words:
            ...
            assert result.query.replace('-','')==a[result.query_start:result.query_end]
            assert result.target.replace('-','')==b[result.target_start:result.target_end]
            score=sum(-2 if '-' in (x,y) else 2 if x==y else -1 for x,y in zip(result.query,result.target))
...
def test_empty_and_limits():
    ...
    with pytest.raises(ValueError):align('A','C','invalid')
    with pytest.raises(ValueError):align('AAA','CCC',max_cells=2)
    with pytest.raises(ValueError):align('A','A',gap=1)

def test_motif_background_and_seed():
    top=motif_enrichment(['AAAAAA'],['CCCCCC'],k=2)[0]
    assert top['kmer']=='AA' and top['log2_enrichment']>0
    assert seed_search('ACGT',{'same':'ACGT','different':'AAAA'},k=2)[0]['id']=='same'
