...
def test_feature_selection_and_scaling_stay_inside_training_folds():
    ...
    results = cross_validate(expression_pipeline(k=5), x, y, cv=cv, return_estimator=True)
    for (train, test), fitted in zip(cv, results['estimator']):
        assert fitted.named_steps['scale'].n_samples_seen_ == len(train)
        chosen = fitted.named_steps['select'].transform(fitted.named_steps['variance'].transform(x[train]))
        assert np.allclose(fitted.named_steps['scale'].mean_, chosen.mean(axis=0))
        assert len(fitted.predict(x[test])) == len(test)


@pytest.mark.parametrize('bad_row', [('missing', 'g1', 3), ('s1', 'missing', 3), ('s1', 'g1', -1), ('s1', 'g1', 1.5)])
