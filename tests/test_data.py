...
from biolab.genomics import analyze_vcf,allele_counts,variant_type
...
def test_vcf_genotypes_filters_and_multiallelic():
    ...
    assert table.iloc[0].sample_alt_frequency==.75
    multi=table[table.pos==10]
    assert list(multi.sample_alt_frequency)==[.5,.5]
    assert summary['transitions']==2 and summary['transversions']==1
    assert variant_type('A','<DEL>')=='structural_or_symbolic'
    with pytest.raises(ValueError):allele_counts({'s':{'GT':'0/3'}},1)

def test_empty_vcf_rejected(tmp_path):
    p=tmp_path/'x.vcf';p.write_text('toy\t1\t.\tA\tG\t1\tPASS\t.\n')
    with pytest.raises(ValueError):list(read_vcf(p))

def test_duplicate_fasta_rejected(tmp_path):
    p=tmp_path/'x.fa';p.write_text('>a\nACG\n>a\nTTT\n')
    with pytest.raises(ValueError):list(read_fasta(p))

def test_bh_known_values_and_order():
    assert np.allclose(benjamini_hochberg([.01,.04,.03,.002]),[.02,.04,.04,.008])
    assert len(benjamini_hochberg([]))==0
    for p in [[-1],[2],[np.nan],[np.inf],[[.5]]]:
        with pytest.raises(ValueError):benjamini_hochberg(p)
...
def test_counts_alignment_and_validation():
    ...
    with pytest.raises(ValueError):validate_counts(counts+.5,meta)
    with pytest.raises(ValueError):validate_counts(counts*0,meta)
    with pytest.raises(ValueError):validate_counts(counts,meta.rename(index={'a':'c'}))
