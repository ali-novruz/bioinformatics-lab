...
def test_enrichment_matches_fisher_and_tests_zero_overlap_terms():
    universe=set(range(20));selected={0,1,2,3}
...
def test_empty_selection_and_unmeasured_selected_gene():
    ...
    with pytest.raises(ValueError):over_representation({9},range(5),{})
...
def test_circular_orf_coordinates_reconstruct_translation(reverse):
    ...
    hits=[r for r in find_orfs(sequence,min_aa=2,circular=True) if r['strand']==strand and r['wraps_origin']]
    assert len(hits)==1
    row=hits[0]
    parts=[sequence[a:b] for a,b in row['segments0']]
    coding=''.join(str(Seq(p).reverse_complement()) for p in parts) if reverse else ''.join(parts)
    assert coding=='ATGAAATAA'
    assert row['protein']=='MK'
    assert not any(r['strand']==strand for r in find_orfs(sequence,min_aa=2,circular=False))


def test_ambiguous_or_empty_orf_input_rejected():
    for sequence in ['', 'ATGNNTAA']:
        with pytest.raises(ValueError):find_orfs(sequence)


def test_phylogeny_known_split_and_complete_deletion():
    records=[SeqRecord(Seq(s),id=n) for n,s in [('a','AAAA-N'),('b','AAAAAN'),('c','TTTTAN'),('d','TTTTAN')]]
