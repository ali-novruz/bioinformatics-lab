...
for tool in bwa samtools bcftools cutadapt; do command -v "$tool" >/dev/null; done
...
if [[ ${SKIP_QC:-0} != 1 ]]; then mkdir "$out/qc_raw" "$out/qc_clean"; fastqc -t "$threads" -o "$out/qc_raw" "$r1" "$r2"; fi
...
if [[ ${SKIP_QC:-0} != 1 ]]; then fastqc -t "$threads" -o "$out/qc_clean" "$out/R1.clean.fq.gz" "$out/R2.clean.fq.gz"; fi
