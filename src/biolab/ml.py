...
def disease_classification(
    ...
    train, test = train_test_split(
        np.arange(len(X)), test_size=0.25, stratify=y, random_state=seed
    )
    models = {
        ...
        "logistic": make_pipeline(
            SimpleImputer(),
            StandardScaler(),
            LogisticRegression(C=1, max_iter=5000, random_state=seed),
        ),
        ...
        rows.append(
            {
                "model": name,
                **{
                    key: float(s["test_" + key].mean())
                    for key in ["roc_auc", "pr_auc", "balanced_accuracy"]
                },
                "auc_fold_sd": float(s["test_roc_auc"].std(ddof=1)),
            }
        )
    cv_table = pd.DataFrame(rows).sort_values(
        ["roc_auc", "model"], ascending=[False, True]
    )
    best = cv_table.iloc[0]["model"]
    model = models[best]
    ...
    summary = {
        ...
        "selected_model": best,
        "test_roc_auc": float(roc_auc_score(truth, p)),
        "test_pr_auc": float(average_precision_score(truth, p)),
        "test_balanced_accuracy": float(balanced_accuracy_score(truth, pred)),
        "test_brier": float(brier_score_loss(truth, p)),
        "sensitivity": float(tp / (tp + fn)),
        "specificity": float(tn / (tn + fp)),
        "confusion_matrix": [[int(tn), int(fp)], [int(fn), int(tp)]],
        "test_auc_bootstrap_95": bootstrap_auc(truth, p, seed=seed),
    }
    canonical = (
        X.assign(malignant=y)
    ...
    predictions = pd.DataFrame(
        {"sample_index": test, "malignant": truth, "probability": p, "prediction": pred}
    )
    test_indices = set(test)
    split = pd.DataFrame(
        {
            "sample_index": np.arange(len(X)),
            "split": ["test" if i in test_indices else "train" for i in range(len(X))],
        }
    )
