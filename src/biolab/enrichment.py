...
def over_representation(
    ...
    if not universe or not foreground <= universe:
        raise ValueError(
            "A nonempty universe containing all selected genes is required"
        )
    if min_size < 1:
        ...
        rows.append(
            {
                ...
                "pvalue": float(
                    hypergeom.sf(
                        len(overlap) - 1, len(universe), len(members), len(foreground)
                    )
                ),
