...
def combine_feature_counts(samples_path):
    ...
    for row in samples.itertuples():
        path = Path(row.counts_path)
        if not path.is_absolute():
            path = samples_path.parent / path
        table = pd.read_csv(path, sep='\t', comment='#', dtype=str, keep_default_na=False)
        if len(table.columns) != 7 or list(table.columns[:6]) != ['Geneid', *ANNOTATION]:
            raise ValueError('Expected Geneid, Chr, Start, End, Strand, Length and one count column')
        if table.empty or table.Geneid.duplicated().any() or table.Geneid.str.strip().eq('').any():
        ...
        if reference is not None and not annotation.equals(reference):
            raise ValueError('Gene universes or coordinates/strand/length differ; check annotations')
        reference = annotation
        values = table.iloc[:, -1]
        # Direct integer parsing avoids rounding large counts through float64.
        if not values.str.fullmatch(r'\d+').all():
            raise ValueError('Raw nonnegative integer counts required (fractional counts unsupported)')
