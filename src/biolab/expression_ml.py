...
def expression_pipeline(k=100, c=1.0, seed=42):
    # Every fitted preprocessing stage remains inside each CV training fold.
    return Pipeline([
        ('variance', VarianceThreshold()),
        ('select', SelectKBest(f_classif, k=k)),
        ('scale', StandardScaler()),
        ('classifier', LinearSVC(C=c, class_weight='balanced', dual='auto',
                                 max_iter=20000, random_state=seed)),
    ])


def fit_training_only(xtrain, ytrain, seed=42):
    if min(pd.Series(ytrain).value_counts()) < 5:
        raise ValueError('Five-fold CV requires at least five training samples per class')
    ...
    search = GridSearchCV(
        ...
        {'select__k': [25, 100, 500], 'classifier__C': [.01, .1, 1.]},
