...
import hashlib,json,platform,uuid,subprocess,os,sys,time
from contextlib import contextmanager

def write_json(path,value):
    path=Path(path)
    staged=path.with_name(path.name+'.tmp')
    staged.write_bytes((json.dumps(value,indent=2,ensure_ascii=False,allow_nan=False)+'\n').encode('utf-8'))
    staged.replace(path)
...
def input_record(root,path):
    ...
    return {'path':path.relative_to(root).as_posix() if local else str(path),
            'external':not local,'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'bytes':path.stat().st_size}
...
def new_run(root,project,inputs=(),parameters=None):
    ...
    code_files=sorted(list((Path(root)/'src').rglob('*.py'))+list((Path(root)/'scripts').glob('*.py'))+list((Path(root)/'workflows').glob('*.sh'))+[p for p in [Path(root)/'pyproject.toml',Path(root)/'projects/registry.json'] if p.exists()])
    source_hashes={str(p.relative_to(root)):hashlib.sha256(p.read_bytes()).hexdigest() for p in code_files}
    write_json(out/'run.json',{'project':project,'created_utc':stamp,'python':platform.python_version(),'platform':platform.platform(),'packages':packages,'git_commit':commit.stdout.strip() if commit.returncode==0 else None,'working_tree_dirty':bool(dirty.stdout.strip()),'source_files_sha256':source_hashes,'command':[sys.executable,*sys.argv],'parameters':parameters or {},'inputs':[input_record(root,p) for p in inputs]})
    receipt=os.environ.get('BIOLAB_RUN_RECEIPT')
    if receipt:
        with Path(receipt).open('a',encoding='utf-8',newline='\n') as handle:
            handle.write(json.dumps({'run_path':out.relative_to(root).as_posix()})+'\n')
    return out


def finish_run(out, *, status, elapsed_seconds, error=None):
    ...
    record.update({'status':status,'finished_utc':datetime.now(timezone.utc).isoformat(),
                   ...
                   'outputs_sha256':{p.relative_to(out).as_posix():hashlib.sha256(p.read_bytes()).hexdigest()
                                     for p in sorted(out.rglob('*')) if p.is_file() and p.name!='run.json'}})
    write_json(out/'run.json',record)


@contextmanager
def recorded_run(root,project,inputs=(),parameters=None):
    out=new_run(root,project,inputs,parameters)
    start=time.perf_counter()
    try:
        yield out
    except BaseException as exc:
        finish_run(out,status='failed',elapsed_seconds=time.perf_counter()-start,error=str(exc))
        raise
    else:
