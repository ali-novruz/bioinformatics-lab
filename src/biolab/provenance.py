...
def verify_outputs(out: str | Path, record: Mapping[str, Any]) -> int:
...
def verify_sources(root: str | Path, record: Mapping[str, Any]) -> dict[str, int]:
    root = Path(root)
        ...
        for entry in json.loads(manifest.read_text(encoding="utf-8"))["archives"]:
            ...
            with zipfile.ZipFile(path) as z:
                for name in z.namelist():
                    archived.setdefault(name, set()).add(
