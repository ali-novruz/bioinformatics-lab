...
def check_health(root: str | Path) -> dict[str, int]:
    root = Path(root)
    load_registry(root)
    manifest = json.loads(
        (root / "datasets/examples/manifest.json").read_text(encoding="utf-8")
    )
    seen = set()
    for item in manifest["files"]:
        ...
        if (
            not item["source_url"].startswith("https://")
            or not item["transformation"].strip()
        ):
    ...
    for node in graph["nodes"]:
        ...
        nodes[node["id"]] = node
    dependencies: dict[str, list[str]] = {name: [] for name in nodes}
    ...
    def visit(name: str) -> None:
        if name in active:
            raise ValueError("Cycle in research dependency graph")
        if name in visited:
            return
        active.add(name)
    ...
    for name in nodes:
        visit(name)
    return {
        "projects": len(load_registry(root)),
        "example_inputs": len(seen),
        "research_nodes": len(nodes),
