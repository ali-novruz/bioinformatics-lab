...
def repository_file(root: str | Path, name: str) -> Path:
...
def load_registry(root: str | Path) -> list[dict[str, Any]]:
    root = Path(root)
    projects = json.loads(
        (root / "projects/registry.json").read_text(encoding="utf-8")
    )["projects"]
    ...
    for project in projects:
        if (
            ...
            or not {"id", "title", "script", "args", "offline", "fetch", "docs"}
            <= project.keys()
        ):
        ...
        for field in ["args", "fetch"]:
            if not isinstance(project[field], list) or not all(
                isinstance(s, str) for s in project[field]
            ):
                raise ValueError(f"{identifier}: {field} must be a string list")
        if (
            type(project["offline"]) is not bool
            or type(project.get("research", False)) is not bool
        ):
        ...
        if Path(project["script"]).name != project["script"] or not project[
            "script"
        ].endswith(".py"):
