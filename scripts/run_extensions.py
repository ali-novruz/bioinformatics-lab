...
def enrichment():
    ...
    out = new_run(
        ...
        [source, annotations],
        {
            "foreground": "padj < 0.05",
            "universe": "finite padj genes",
            "test": "hypergeometric upper tail",
            "correction": "BH across all eligible GO slim BP terms",
            "min_term_size": 2,
        },
    )
    de = pd.read_csv(source).set_index("gene_id")
    universe = set(de.index[np.isfinite(de.padj)])
    foreground = set(de.index[de.padj < 0.05])
    ...
    pd.Series(sorted(universe), name="gene_id").to_csv(
        out / "universe.csv", index=False
    )
    pd.Series(sorted(foreground), name="gene_id").to_csv(
    ...
    summary = {
        ...
        "foreground_genes": len(foreground),
        "annotated_universe_genes": len(annotated),
        "unannotated_universe_genes": len(universe - annotated),
        "tested_terms": len(table),
        "significant_terms_fdr_005": int((table.padj < 0.05).sum()),
    ...
    ax.barh(top.term_name, -np.log10(top.pvalue.clip(lower=1e-300)), color="#157f83")
    ax.set(
        xlabel="-log10 raw enrichment p-value",
        title=f"GO-slim: {len(foreground)} selected gene; {summary['significant_terms_fdr_005']} FDR discoveries",
    )
    fig.tight_layout()
    fig.savefig(out / "enrichment.png", dpi=150)
    plt.close(fig)
    return out
...
def phylogeny():
    ...
    out = new_run(
        ...
        [source],
        {
            ...
            "gaps": "complete deletion across all taxa",
            "tree": "neighbor joining, unrooted",
            "bootstrap": 200,
            "seed": 42,
        },
    )
    ...
    pd.DataFrame(
        {"alignment_column1": np.arange(1, len(mask) + 1), "retained": mask}
    ).to_csv(out / "alignment_columns.csv", index=False)
    write_json(out / "bootstrap_splits.json", support)
    negative = sum(
        c.branch_length is not None and c.branch_length < 0 for c in tree.find_clades()
    )
    summary = {
        "real_sequence_fragments": True,
        "taxa": len(names),
        "alignment_columns": len(mask),
        "complete_columns": int(mask.sum()),
        "removed_columns": int((~mask).sum()),
        "variable_complete_columns": int(np.any(matrix != matrix[0], axis=0).sum()),
        "bootstrap_replicates": 200,
        "negative_branch_lengths": negative,
    ...
    ax.set(
        title="Opuntia fragments: unrooted NJ, 200 site bootstraps",
        xlabel="p-distance branch length (display root is arbitrary)",
    )
    fig.tight_layout()
...
def orfs():
    ...
    with (out / "proteins.fasta").open("w", encoding="utf-8", newline="\n") as handle:
        for i, row in enumerate(found, 1):
            name = f"orf_{i:03}"
            handle.write(
                f">{name} strand={row['strand']} candidate_only\n{row['protein']}\n"
            )
            rows.append(
                {
                    "orf_id": name,
                    **{k: v for k, v in row.items() if k != "protein"},
                    "segments0": json.dumps(row["segments0"]),
                }
            )
    ...
    write_json(
        out / "summary.json",
        {
            ...
            "length_nt": len(record),
            "candidate_orfs": len(found),
            "plus_strand": sum(r["strand"] == "+" for r in found),
            "minus_strand": sum(r["strand"] == "-" for r in found),
            "origin_spanning": sum(r["wraps_origin"] for r in found),
            "longest_aa": max((r["length_aa"] for r in found), default=0),
    ...
    for i, row in enumerate(found):
        for start, end in row["segments0"]:
            ax.plot(
                ...
                [i, i],
                lw=3,
                color="#157f83" if row["strand"] == "+" else "#c46a45",
            )
    ax.set(
        xlabel="Reference position (0-based)",
        ylabel="ORF candidate",
        title="PhiX174 circular ORFs: teal + / orange -",
    )
    fig.tight_layout()
    fig.savefig(out / "orfs.png", dpi=150)
    plt.close(fig)
    return out
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project", choices=[*RUNNERS, "all"], required=True)
