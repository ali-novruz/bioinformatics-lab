...
if __name__ == "__main__":
    ...
    p.add_argument("--counts", required=True, help="genes x samples TSV")
    p.add_argument("--metadata", required=True, help="CSV indexed by sample ID")
    p.add_argument("--design", default="~ type + condition")
    p.add_argument("--dataset-name", default="User-supplied RNA-seq")
    a = p.parse_args()
    root = Path(__file__).resolve().parents[1]
    counts = pd.read_csv(a.counts, sep="\t", index_col=0).T
    ...
    out = new_run(
        ...
        parameters={
            ...
            "counts": str(Path(a.counts).resolve()),
    ...
    write_json(
        out / "input_hashes.json",
        {
            ...
            for p in [a.counts, a.metadata]
        },
    )
    res, norm, summary = differential_expression(counts, meta, a.design)
    res.to_csv(out / "differential_expression.csv")
    norm.to_csv(out / "normalized_counts.csv")
    write_json(out / "summary.json", summary)
    plot_expression(res, norm, meta, out, dataset=a.dataset_name, design=a.design)
    print(out)
