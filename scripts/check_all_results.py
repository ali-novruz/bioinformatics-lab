...
def main():
    record_count=0; archived=0; current=0
    for record_path in (ROOT/'results').rglob('run.json'):
        if 'runs' in record_path.relative_to(ROOT/'results').parts:continue
    ...
    for path in (ROOT/'results').rglob('snapshot.json'):
        if 'runs' in path.relative_to(ROOT/'results').parts:continue
        for name,expected in json.loads(path.read_text(encoding='utf-8'))['sha256'].items():
            assert hashlib.sha256((path.parent/name).read_bytes()).hexdigest()==expected,(path,name)
    print(f'{record_count} published runs; {current} current and {archived} archived source matches; example inputs and all snapshots verified')
