...
def validate(root):
    ...
    expected = "\t".join(
        map(
            str,
            [truth["chrom"], truth["position"], truth["ref"], truth["alt"], "PASS", 1],
        )
    )
    if variants != [expected]:
        raise ValueError(
            f"Expected exactly the planted passing haploid SNV {expected}; observed {variants}"
        )
    ...
    observed = next(
        int(list(row.values())[-1]) for row in rows if row["Geneid"] == truth["gene"]
    )
    if observed < 0.95 * truth["pairs"]:
        raise ValueError(
            f"Insufficient assigned fragments: {observed} / {truth['pairs']}"
        )
    summary = {
        "variant_matches_truth": True,
        "assigned_fragments": observed,
        "input_pairs": truth["pairs"],
    }
