...
from biolab.expression_ml import load_khan, fit_training_only
...
def main():
    ...
    out = new_run(ROOT, 'gene-expression-ml', sorted(folder.glob('*.csv')),
                  {'seed': 42, 'k_grid': [25, 100, 500], 'C_grid': [.01, .1, 1.],
    ...
    dummy_cv = cross_val_score(DummyClassifier(strategy='most_frequent'), xtrain, ytrain,
    ...
    summary = {
        ...
        'features': 2308, 'train_samples': 63, 'test_samples': 20,
        'train_class_counts': {str(k): int(v) for k, v in ytrain.value_counts().sort_index().items()},
        'test_class_counts': {str(k): int(v) for k, v in ytest.value_counts().sort_index().items()},
        ...
        'dummy_test_balanced_accuracy': float(balanced_accuracy_score(ytest, dummy.predict(xtest))),
        'class_order': [1, 2, 3, 4],
        'confusion_matrix': confusion_matrix(ytest, prediction, labels=[1, 2, 3, 4]).tolist(),
        'limitations': [
    ...
    bootstrap = [float(rng.choice(correct, len(correct), replace=True).mean()) for _ in range(2000)]
    summary['test_accuracy_bootstrap_95_conditional'] = np.quantile(bootstrap, [.025, .975]).tolist()
    write_json(out / 'summary.json', summary)
    pd.DataFrame(search.cv_results_).to_csv(out / 'cv_results.csv', index=False)
    pd.DataFrame({'sample_id': [f'test_{i+1:02}' for i in range(len(ytest))],
                  'truth': ytest, 'prediction': prediction}).to_csv(out / 'predictions.csv', index=False)
    report = classification_report(ytest, prediction, labels=[1, 2, 3, 4], output_dict=True, zero_division=0)
    ...
    pd.DataFrame({'probe_column': selected,
                  'training_anova_score': selector.scores_[selector.get_support()]}).to_csv(out / 'selected_probes.csv', index=False)
    fig, ax = plt.subplots(figsize=(6, 5))
    ConfusionMatrixDisplay.from_predictions(ytest, prediction, labels=[1, 2, 3, 4], cmap='Blues', ax=ax, colorbar=False)
    ...
    ax.bar(['Dummy CV', 'Selected SVM CV', 'SVM test'],
           [summary['dummy_cv_balanced_accuracy'], summary['selected_cv_balanced_accuracy'], summary['test_balanced_accuracy']],
           color=['#a1abb2', '#2c7c88', '#ac5a44'])
    ax.set(ylim=(0, 1.08), ylabel='Balanced accuracy', title='Training selection and held-out evaluation')
