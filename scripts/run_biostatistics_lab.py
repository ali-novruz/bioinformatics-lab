...
if __name__ == '__main__':
    ...
    out = new_run(ROOT, 'biostatistics-lab', [source],
                  {'seed': 20260916, 'null_genes': 1000, 'samples_per_group': 20,
    ...
    pd.DataFrame({'synthetic_gene': np.arange(1000), 'p': p, 'padj_bh': adjusted}).to_csv(out / 'null_tests.csv', index=False)
    summary = {
        'simulation': 'All 1000 null hypotheses are true by construction; one seeded realization',
        'uncorrected_p_below_005': int((p < .05).sum()),
        'bh_adjusted_p_below_005': int((adjusted < .05).sum()),
        'expected_uncorrected_false_positives_if_calibrated': 50,
        'library_mean': float(libraries.mean()), 'library_median': float(libraries.median()),
    ...
    axes[1].bar(['p < .05', 'BH < .05'], [summary['uncorrected_p_below_005'], summary['bh_adjusted_p_below_005']], color=['#ac5a44', '#2c7c88'])
    axes[1].set(title='False positives in one seeded simulation', ylabel='Tests selected')
    fig.tight_layout(); fig.savefig(out / 'multiple_testing.png', dpi=150); plt.close(fig)
    print(out)
    print(summary)
