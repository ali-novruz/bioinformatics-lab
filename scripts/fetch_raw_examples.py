...
def fetch(dataset, output):
    ...
    selected = [entry for entry in manifest['files'] if dataset == 'all' or entry['dataset'] == dataset]
    for entry in selected:
        target = output / entry['path']
        if target.exists():
            if hashlib.sha256(target.read_bytes()).hexdigest() != entry['sha256']:
                raise ValueError(f'Existing input changed: {target}. Restore it or choose a new directory.')
            continue
    ...
    (output / 'verified-manifest.json').write_text(json.dumps({**manifest, 'files': selected}, indent=2) + '\n')
    print(f'Verified {len(selected)} inputs in {output}')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--dataset', choices=['rnaseq', 'variants', 'all'], default='all')
