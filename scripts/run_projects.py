...
from biolab.sequence import align, sequence_summary
from biolab.transcriptomics import differential_expression, load_pasilla
...
def run_dna():
    ...
    if not source.exists():
        source = ROOT / "datasets/fixtures/toy.fasta"
    ...
    write_json(
        out / "summary.json",
        {
            ...
            "real_data": source.parent == RAW,
            "records": records,
        },
    )
    fig, ax = plt.subplots(figsize=(7, 4))
    ax.bar(list(records), [r["gc_fraction_acgt_only"] or 0 for r in records.values()])
    ax.set(
        ylabel="GC fraction (ACGT denominator)",
        ylim=(0, 1),
        title="Sequence composition",
    )
    fig.tight_layout()
    fig.savefig(out / "gc.png", dpi=150)
    plt.close(fig)
    return out
...
def run_alignment():
    ...
    out = new_run(
        ROOT, "alignment", parameters={"match": 2, "mismatch": -1, "linear_gap": -2}
    )
    result = []
    for mode in ["global", "local"]:
        ...
        if ours.score != score:
            raise AssertionError("Biopython score differs")
        result.append({**ours.to_dict(), "biopython_score": score})
    write_json(
        out / "summary.json",
        {"data": "synthetic teaching sequences", "alignments": result},
    )
    return out
...
def run_variants():
    ...
    if not source.exists():
        raise FileNotFoundError("Run fetch_data.py --dataset giab first")
    ...
    fig, ax = plt.subplots(figsize=(7, 4))
    table["type"].value_counts().plot.bar(ax=ax, color="#157f83")
    ax.set(ylabel="Retained ALT alleles", title="GIAB HG001: first 1000 records only")
    fig.tight_layout()
    fig.savefig(out / "variant_types.png", dpi=150)
    plt.close(fig)
    return out


def plot_expression(
    res, norm, meta, out, *, dataset="Pasilla", design="~ type + condition"
):
    log = np.log1p(norm)
    ...
    for idx, name in enumerate(log.index):
        ax.annotate(
            name,
    ...
    ax.set(xlabel="PC1", ylabel="PC2", title=f"{dataset} PCA: log1p normalized counts")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("Top 30 variable genes; row z-score")
    fig.colorbar(im, ax=ax)
    fig.tight_layout()
    fig.savefig(out / "heatmap.png", dpi=150)
    plt.close(fig)
    valid = res.dropna(subset=["padj", "log2FoldChange"])
    sig = (valid.padj < 0.05) & (valid.log2FoldChange.abs() > 1)
    fig, ax = plt.subplots(figsize=(7, 5))
    ax.scatter(
        valid.log2FoldChange,
        -np.log10(valid.padj.clip(lower=1e-300)),
        c=np.where(sig, "#c23b4a", "#75878b"),
        s=7,
        alpha=0.6,
    )
    ax.axhline(-np.log10(0.05), ls="--", c="black", lw=0.7)
    ax.set(
        xlabel="log2FC treated / untreated (unshrunken)",
        ylabel="-log10 adjusted p",
        title=f"{dataset}: {design}",
    )
    fig.tight_layout()
...
def run_rnaseq():
    ...
    results.to_csv(out / "differential_expression.csv", index_label="gene_id")
...
def run_ml():
    out = new_run(
        ROOT, "ml", parameters={"seed": 42, "test_fraction": 0.25, "cv_folds": 5}
    )
    ...
    fig, ax = plt.subplots()
    RocCurveDisplay.from_predictions(pred.malignant, pred.probability, ax=ax)
    ax.set_title("Held-out WDBC: malignant positive")
    fig.tight_layout()
    fig.savefig(out / "roc.png", dpi=150)
    plt.close(fig)
    fig, ax = plt.subplots()
    ConfusionMatrixDisplay.from_predictions(
        pred.malignant, pred.prediction, display_labels=["benign", "malignant"], ax=ax
    )
    fig.tight_layout()
