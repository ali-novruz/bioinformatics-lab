...
def rna_plots(results, normalized, metadata, output):
    ...
    for group, label in [('untreated', 'Rap1-AID uninduced'), ('treated', 'Rap1-AID + IAA 30 min')]:
    ...
    for index, sample in enumerate(log_counts.index):
        ax.annotate(sample, coordinates[index], fontsize=7, xytext=(3, 5), textcoords='offset points')
    ...
    ax.margins(.2); ax.legend(fontsize=8)
    fig.tight_layout(); fig.savefig(output / 'pca.png', dpi=150); plt.close(fig)
    ...
    fig.colorbar(im, ax=ax); fig.tight_layout(); fig.savefig(output / 'heatmap.png', dpi=150); plt.close(fig)
    valid = results.dropna(subset=['padj', 'log2FoldChange'])
    significant = (valid.padj < .05) & (valid.log2FoldChange.abs() > 1)
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.scatter(valid.log2FoldChange, -np.log10(valid.padj.clip(lower=1e-300)),
               c=np.where(significant, '#bf414d', '#5f7e89'), s=15)
    ax.axhline(-np.log10(.05), color='black', ls='--', lw=.7)
    ax.set(xlabel='log2 fold change: IAA 30 min / uninduced (unshrunken)',
           ylabel='-log10 adjusted p', title='GSE110004: chrI subset, exploratory differential expression')
    fig.tight_layout(); fig.savefig(output / 'volcano.png', dpi=150); plt.close(fig)


def rna_example(raw, output):
    metadata = pd.read_csv(ROOT / 'datasets/raw-rnaseq-samples.csv', index_col=0)
    folder = raw / 'rnaseq'
    env = {**os.environ, 'THREADS': '2', 'STAR_SA_INDEX_NBASES': '7', 'SJDB_OVERHANG': '100',
           'SKIP_QC': '0', 'SKIP_MULTIQC': '0'}
    rows = []
    mapping = {}
    for sample in metadata.index:
        sample_out = output / 'rnaseq' / sample
        run(['bash', 'workflows/rnaseq.sh', folder / 'genome.fa', folder / 'genes.gtf',
             folder / f'{sample}_1.fastq.gz', folder / f'{sample}_2.fastq.gz',
             sample, sample_out, 2], env=env)  # Verified reverse-stranded library.
        rows.append({'sample_id': sample, 'counts_path': str(sample_out / f'{sample}.counts.tsv')})
        lines = (sample_out / f'{sample}.Log.final.out').read_text().splitlines()
        star = {line.split('|', 1)[0].strip(): line.split('|', 1)[1].strip() for line in lines if '|' in line}
        input_pairs = int(star['Number of input reads'])
        unique = int(star['Uniquely mapped reads number'])
        if input_pairs != 50000 or unique < input_pairs * .5:
            raise ValueError(f'Unexpected read count or poor mapping: {sample}: {star}')
        mapping[sample] = {'input_pairs': input_pairs, 'uniquely_mapped_pairs': unique,
                           'uniquely_mapped_percent': star['Uniquely mapped reads %']}
    model = output / 'rna-model'; model.mkdir()
    samples_path = model / 'counts_manifest.tsv'
    pd.DataFrame(rows).to_csv(samples_path, sep='\t', index=False)
    run([sys.executable, 'scripts/combine_counts.py', '--samples', samples_path, '--output', model / 'counts.tsv'])
    ...
    results, normalized, summary = differential_expression(counts, metadata, design='~ condition')
    ...
    summary.update(dataset='GSE110004, nf-core chromosome I subsample',
                   ...
                   mapping=mapping, assigned_fragments=counts.sum(axis=1).astype(int).to_dict())
...
def variant_example(raw, output):
    folder = raw / 'variants'
    combined = output / 'variant-input'; combined.mkdir()
    for mate in (1, 2):
        with (combined / f'R{mate}.fastq.gz').open('wb') as writer:
            for lane in ['L001', 'L002', 'L004', 'L007', 'L008']:
                with (folder / f'tiny_n_{lane}_R{mate}_xxx.fastq.gz').open('rb') as reader:
                    shutil.copyfileobj(reader, writer)
    # Concatenated gzip members are valid gzip and preserve all five lanes.
    variant_out = output / 'variants'
    run(['bash', 'workflows/variants.sh', folder / 'reference.fa', combined / 'R1.fastq.gz',
         combined / 'R2.fastq.gz', 'sarek-tiny-normal', variant_out, 2],
        env={**os.environ, 'THREADS': '2', 'SKIP_QC': '0', 'SKIP_MULTIQC': '0'})
    mapped = int(subprocess.check_output(['samtools', 'view', '-c', '-F', '4', variant_out / 'aligned.bam'], text=True))
    if mapped == 0:
        raise ValueError('External DNA fixture produced no mapped reads')
    records = subprocess.check_output(['bcftools', 'query', '-f', '%CHROM\t%POS\t%REF\t%ALT\t%FILTER\n',
                                      variant_out / 'filtered.vcf.gz'], text=True).splitlines()
    summary = {'dataset': 'nf-core/sarek tiny normal, five lanes', 'mapped_reads': mapped,
               'variant_records': len(records), 'pass_records': sum(line.endswith('\tPASS') for line in records),
...
if __name__ == '__main__':
    ...
    scripts = list((ROOT / 'scripts').glob('*.py')) + list((ROOT / 'workflows').glob('*.sh')) + list((ROOT / 'src/biolab').glob('*.py'))
    save_json(output / 'run.json', {
        'git_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip(),
        'github_run_id': os.environ.get('GITHUB_RUN_ID'),
        'python': sys.version,
        'source_files_sha256': {p.relative_to(ROOT).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest() for p in scripts},
    })
    with (output / 'python-environment.txt').open('w') as handle:
        run([sys.executable, '-m', 'pip', 'freeze'], stdout=handle)
    summary = {'variants': variant_example(raw, output), 'rnaseq': rna_example(raw, output)}
