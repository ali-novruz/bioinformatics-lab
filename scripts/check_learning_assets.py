...
def main():
    catalog = json.loads((ROOT / 'resources/unec/catalog.json').read_text(encoding='utf-8'))
    files = catalog['files']
    ...
    books = json.loads((ROOT / 'resources/books/pdf-manifest.json').read_text(encoding='utf-8'))['books']
    for book in books:
        path = ROOT / book['path']
